...
export const dailyBriefing = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ data: ext }, { data: stats }, { data: sessions }, { data: metrics }] = await Promise.all([
      ...
      supabase.from("workout_sessions").select("completed_at,duration_min,calories").eq("user_id", userId).not("completed_at", "is", null).gt("duration_min", 0).gte("completed_at", since).order("completed_at", { ascending: false }),
        ...
        tool: {
          ...
          parameters: {
            ...
            properties: {
              ...
              carbs_g: { type: "number" },
              fat_g: { type: "number" },
              fibre_g: { type: "number", description: "Estimated dietary fibre in grams" },
              ...
              possible_allergens: {
                type: "array",
                items: { type: "string" },
                description: "Common allergens visibly likely in the dish (e.g. dairy, gluten, nuts, egg, shellfish, soy)",
              },
              uncertainty: {
                type: "string",
                description: "What is hardest to judge from the photo and what the member should verify",
              },
...
export const adaptProgram = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ data: ext }, { data: sessions }] = await Promise.all([
      supabase.from("user_profiles_ext").select("fitness_goal,training_level,preferred_type").eq("user_id", userId).maybeSingle(),
      supabase.from("workout_sessions").select("completed_at,duration_min,calories,notes").eq("user_id", userId).not("completed_at", "is", null).gt("duration_min", 0).gte("completed_at", since).order("completed_at"),
    ]);
    const result = await callAI({
      messages: [
...
export const detectPlateau = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: sessions } = await supabase
      .from("workout_sessions").select("completed_at,duration_min,calories").eq("user_id", userId).not("completed_at", "is", null).gt("duration_min", 0)
...
export const weeklyRecap = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ data: sessions }, { data: stats }] = await Promise.all([
      supabase.from("workout_sessions").select("completed_at,duration_min,calories").eq("user_id", userId).not("completed_at", "is", null).gt("duration_min", 0).gte("completed_at", since),
