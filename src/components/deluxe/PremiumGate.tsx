...
import { usePremium, type Tier } from "@/hooks/usePremium";
...
interface Props {
  ...
  feature: string;
  description?: string;
  /** Minimum tier required. Defaults to any paid tier. */
  minTier?: Tier;
}
...
export function PremiumGate({ children, feature, description, minTier }: Props) {
  const { isPremium, hasTier, loading } = usePremium();
  if (loading) {
    return (
  ...
  const allowed = minTier ? hasTier(minTier) : isPremium;
  if (allowed) return <>{children}</>;
  const required = minTier ? TIER_LABEL[minTier] ?? "Signature" : "Essential";
  return (
    <div className="mx-auto max-w-2xl px-5 pt-12">
      <div className="relative overflow-hidden border border-gold/30 bg-gradient-to-br from-deluxe-forest/40 to-deluxe-black p-8 text-center">
        ...
        <div className="relative">
          ...
          <SectionLabel>{required} membership</SectionLabel>
          <h2 className="mt-3 font-display text-3xl text-foreground">{feature}</h2>
          <p className="mx-auto mt-3 max-w-md text-sm text-muted-foreground">
            {description ?? `Unlock this with a ${required} membership.`}
          </p>
          <Link to="/pricing" className="mt-6 inline-block">
            <GoldButton>View {required}</GoldButton>
          </Link>
