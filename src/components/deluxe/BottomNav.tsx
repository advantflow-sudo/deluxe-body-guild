...
export function BottomNav() {
  return (
    <nav aria-label="Primary" className="fixed bottom-0 left-0 right-0 z-40 border-t border-gold/20 bg-deluxe-black/95 backdrop-blur-xl pb-[env(safe-area-inset-bottom)]">
      ...
      <div className="mx-auto flex max-w-2xl items-stretch justify-between px-0.5 py-1.5 pl-[max(0.125rem,env(safe-area-inset-left))] pr-[max(0.125rem,env(safe-area-inset-right))] sm:py-2">
        {items.map((i) => (
          <Link
            key={i.to}
            to={i.to}
